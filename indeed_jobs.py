...
def get_indeed_jobs_by_search_terms(jobs_search_terms, on_site_or_remote_jobs = 'on-site')
	...
	# Create different GET requests for 'on-site' and 'remote' job types
	if on_site_or_remote_jobs == 'on-site':
		url += 'q='
	elif on_site_or_remote_jobs == 'remote':
		# Filter only by the remote jobs
		url += 'remotejob=032b3046-06a3-4876-8dfd-474eb5e7ed11&q='
	else :
		print('Bad input, please select either "on-site" or "remote" without the quotes! Exiting from the program!')
		exit()
