...
type_of_jobs_options = ['all', 'remote', 'both']
...
# Get both all and remote jobs in one chart
if type_of_jobs_selected == 'both':
	# Call function with 'function_name' dynamically
	all_jobs_count = globals()[function_name](jobs_search_terms, 'all')
	remote_jobs_count = globals()[function_name](jobs_search_terms, 'remote')

	visualize_data_in_grouped_bar_chart(jobs_search_terms, all_jobs_count, remote_jobs_count, chart_title)
else:
